...
router.get('/needs/description', (req, res) => {
    ...
    if (req.query && req.query.srcLang) {
        const srcLang = req.query.srcLang;
        const dstLang = domain.split('.')[0];
        if (srcLang === dstLang || !isValidLang(srcLang)) {
        ...
        .then(e => P.join(
            getPageSummary(`${srcLang}.wikipedia.org`,
                encodeURIComponent(e.sitelinks[`${srcLang}wiki`].title)),
            getPageSummary(domain, encodeURIComponent(e.sitelinks[`${dstLang}wiki`].title)),
            (src, dst) => {
                res.status(200).json({
                    src: src.body,
                    dst: dst.body
